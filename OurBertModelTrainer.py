...
class BertModelTrainer():
    ...
    def __call__(self,
        ...
        for epoch in range(epochs):
            ...
            batch_mask_index = [[] for i in range(max_mask_len)]
            batch_mask_label = [[] for i in range(max_mask_len)]
            batch_mask_sentence = [[] for i in range(max_mask_len)]
            batch_real_sentence = [[] for i in range(max_mask_len)]
            batch_segment_sentence = [[] for i in range(max_mask_len)]
            batch_NSP_label = [[] for i in range(max_mask_len)]
            ...
            for index,sentence in enumerate(inputs):
                ...
                pos = len(target_index)-1              
                batch_mask_index[pos].append(target_index)
                batch_mask_label[pos].append(target_label)
                batch_mask_sentence[pos].append(mask_input)
                batch_real_sentence[pos].append(real_sentence)
                batch_segment_sentence[pos].append(target_segment)
                batch_NSP_label[pos].append(target_NSP)

                if len(batch_mask_index[pos]) == batch2Show:
                    batchMaskedSentence = tf.convert_to_tensor(batch_mask_sentence[pos],tf.float32)
                    batchSentenceSegment = tf.convert_to_tensor(batch_segment_sentence[pos],tf.float32)
                    batchRealSentence = tf.convert_to_tensor(batch_real_sentence[pos],tf.float32)
                    batchMaskIndex = tf.convert_to_tensor(batch_mask_index[pos],tf.float32)
                    batchMaskLabel = tf.convert_to_tensor(batch_mask_label[pos],tf.float32)
                    batchNSPLabel = tf.convert_to_tensor(batch_NSP_label[pos],tf.float32)
                    
                    countBatch = countBatch+1
                    total_loss=0
                    ####################### Train Part ####################### 
                    with tf.GradientTape() as tape:
                        MLM_pred = BertModel.train_for_MLM(batchMaskedSentence,batchSentenceSegment,True)
                        ...
                        NSP_pred = BertModel.train_for_NSP(batchRealSentence,batchSentenceSegment,True)
                    ...
                    self.train_accuracy_MLM(batchMaskLabel, MLM_Masked)
                    ...
                    if countBatch%32 == 0:
                        ...
                        print("Epoch:{} Batch:{} Loss_MLM:{:.6f} Accuracy_MLM:{:.6f} mask_len: {}".format(
                            epoch+1,countBatch,self.train_loss_MLM.result(),self.train_accuracy_MLM.result(),pos+1))
                        print("__________________________________________________________")
                        print("Loss_NSP:{:.6f} Accuracy_NSP:{:.6f}".format(
                        self.train_loss_NSP.result(),self.train_accuracy_NSP.result()))
                        print("***********************************************************")

                    batch_mask_index[pos] = []
                    batch_mask_label[pos] = []
                    batch_mask_sentence[pos] = []
                    batch_real_sentence[pos] = []
                    batch_segment_sentence[pos] = []
                    batch_NSP_label[pos] = []        
